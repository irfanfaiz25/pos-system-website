<div x-data="{ isSidebarVisible: @entangle('isSidebarVisible') }">
    ...
    <button @click="isSidebarVisible = !isSidebarVisible"
        class="lg:hidden p-2 fixed top-4 left-4 z-50 dark:text-gray-50">
    ...
    <div :class="isSidebarVisible ? 'translate-x-0' : '-translate-x-full'" {{--
        ...
        --}}
        class="bg-white dark:bg-[#1c1c1c] shadow-lg fixed top-0 left-0 min-h-screen w-[5.5rem] duration-500
        text-gray-100 px-3
        z-40
        ...
        <div class="mt-10 flex flex-col gap-2 relative text-gray-800 dark:text-gray-50">
            ...
            <a href="{{ route($menu['route']) }}" wire:navigate
                class="group flex justify-center items-center text-sm h-11 gap-3.5 font-medium p-2 hover:bg-[#f2f2f2] dark:hover:bg-[#252525] hover:text-green-500 dark:hover:text-green-500 rounded-md {{ request()->is($menu['request']) ? 'bg-[#f2f2f2] dark:bg-[#252525] text-green-500' : 'text-gray-800 dark:text-gray-50' }}">
